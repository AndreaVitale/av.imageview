...
static Handle<Value> ImageviewAndroid_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("ImageviewAndroid.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::ImageviewAndroidBindings::lookupGeneratedInit(
...
static void ImageviewAndroid_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(ImageviewAndroid_getBinding)->GetFunction());
}

static void ImageviewAndroid_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::ImageviewAndroidBindings::lookupGeneratedInit(*binding, bindingLength);
	...
	(JNIEnv *env, jobject self)
{
	...
	titanium::KrollBindings::addExternalLookup(&(::ImageviewAndroidBindings::lookupGeneratedInit));
}
